...
const AssignCourseModal: React.FC<AssignCourseModalProps> = ({ isOpen, onClose, user }) => {
	...
	const [isAssigningSemester, setIsAssigningSemester] = useState<boolean>(false);
	...
	return (
		<div className='fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50'>
			<div className='bg-white p-6 rounded-lg w-full max-w-md max-h-[90vh] overflow-y-auto'>
				<h2 className='text-xl font-bold mb-4 text-gray-800'>Assign Content to {user.name}</h2>
				...
				<div className='border-t border-gray-200 my-4 pt-4 space-y-4'>
					...
					{!selectedSemester ? (
						<div className='text-sm text-gray-500 italic'>Please select a semester first to view available courses</div>
					) : coursesLoading ? (
							...
							<div className='flex flex-col gap-2'>
								...
								>
									...
									{courses && courses.length > 0 ? (
										...
										<option disabled>No courses available for this semester</option>
									)}
								</select>
							</div>
